...
Help ID: `SolidWorks.Interop.sldworks~SolidWorks.Interop.sldworks.IFeatureManager~HoleWizard4`

Obsolete. Superseded by IFeatureManager::HoleWizard5.
Obsolete. Superseded by [IFeatureManager::HoleWizard5](SolidWorks.Interop.sldworks~SolidWorks.Interop.sldworks.IFeatureManager~HoleWizard5.md).
...
*RevDir*
:   True to reverse the direction of the hole, false to not (see **Remarks**)
...
Before creating a hole using a reference plane and IFeatureManager::HoleWizard4, determine the value of the RevDir parameter as follows:
...
The screw fit is defined as [swWzdHoleScrewClearanceTypes\_e](ms-help://SolidWorks.Interop.swconst/SolidWorks/SOLIDWORKS.Interop.swconst~SOLIDWORKS.Interop.swconst.swWzdHoleScrewClearanceTypes_e.html) and only used if a hole diameter is not specified; default value is NORMAL\_FIT. The drill angle defaults to 118, if not specified. Input angles are in radians; input values are in meters.
...
>
> 1. For helicoil holes: Thread depth is calculated based on a constant (determined by hole type i.e. Insert = 1.0 \* diameter) + thread depth from the standard \* thread major diameter.
