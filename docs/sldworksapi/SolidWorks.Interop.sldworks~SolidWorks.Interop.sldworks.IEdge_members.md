...
Help ID: `SolidWorks.Interop.sldworks~SolidWorks.Interop.sldworks.IEdge_members`

Allows access to its defining coedge, and adjacent faces, and its underlying curve and vertices as well as edge data.
The following tables list the members exposed by [IEdge](SolidWorks.Interop.sldworks~SolidWorks.Interop.sldworks.IEdge.md).

Public Properties

|  | Name | Description |
| --- | --- | --- |
| Property | [Check](SolidWorks.Interop.sldworks~SolidWorks.Interop.sldworks.IEdge~Check.md) | Gets whether the edge is a valid, and, if not, returns the faults. |
